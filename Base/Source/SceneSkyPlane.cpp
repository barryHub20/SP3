...
void SceneSkyPlane::Init()
{
	...
	meshList[GEO_OBJECT] = MeshBuilder::GenerateOBJ("OBJ1", "OBJ//lamppost.obj");//MeshBuilder::GenerateCube("cube", 1);
	meshList[GEO_OBJECT]->textureID[0] = LoadTGA("Image//calibri.tga");
	...
	meshList[GEO_SKYPLANE] = MeshBuilder::GenerateSkyPlane("GEO_SKYPLANE", Color(1, 1, 1), 128, 6.0f, 60.0f, 6.0f, 6.0f);
	...
	meshList[GEO_GROUND]->textureID[1] = LoadTGA("Image//moss1.tga"); 
	...
	/*GEO_POLICE_CAR,
		GEO_POLICE_CAR_SIREN,
		GEO_POLICE_CAR_WINDOW,*/
...
void SceneSkyPlane::UpdateOpenGL(double dt)
{
...
void SceneSkyPlane::UpdateLight(double dt)
{
...
void SceneSkyPlane::UpdatePlayable(double dt)
{
	//Lab prac 2
...
void SceneSkyPlane::UpdateBullet(double dt)
{
	//update
	static bool pressedDown = false;
	if(Application::IsKeyPressed('M') && !pressedDown)
	{
		pressedDown = true;
		for(int i = 0; i < bulletSize; ++i)
		{
...
void SceneSkyPlane::RenderLight()
{
...
void SceneSkyPlane::Render3DObjects()
{
	/* axes */
	...
	modelStack.Rotate(-90, 1, 0, 0);
	modelStack.Scale(8000.f, 8000.f, 1.f);
...
void SceneSkyPlane::RenderBullet()
{
	for(int i = 0; i < bulletSize; ++i)
	{
		if(ammo[i].GetStatus())
...
void SceneSkyPlane::RenderPoliceCar()
{
	modelStack.PushMatrix();	//main body
		
		modelStack.PushMatrix();	//siren
		modelStack.PopMatrix();

		modelStack.PushMatrix();	//glass
		modelStack.PopMatrix();

	modelStack.PopMatrix();
}

void SceneSkyPlane::UpdateCameraStatus(const unsigned char key)
{
