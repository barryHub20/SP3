...
*/
/******************************************************************************/
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, Color color, float length, float texCoord)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
...
Mesh* MeshBuilder::GenerateDebugCube(const std::string &meshName, Color color, float length)
{
	...
	v.pos.Set(-0.5f * length,-0.5f * length,-0.5f * length);//0
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f * length,-0.5f * length,-0.5f * length);	//1
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f * length, 0.5f * length,-0.5f * length);	//2
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f * length, 0.5f * length,-0.5f * length);//3
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f * length,-0.5f * length, 0.5f * length);//4
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f * length,-0.5f * length, 0.5f * length);//5
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f * length, 0.5f * length, 0.5f * length);//6
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f * length, 0.5f * length, 0.5f * length);//7
	v.color = color;
	vertex_buffer_data.push_back(v);
...
Mesh* MeshBuilder::GenerateTerrain(const std::string &meshName, const std::string &file_path, std::vector<unsigned char> &heightMap)
{
	...
	/* normal */
	Vector3 pos;	//current pos of current element
	Vector3 front;	//front vector
	Vector3 frontPos;	//pos of front element
	Vector3 right;	//right vector
	Vector3 rightPos;	//pos of right element
	Vector3 top;
	...
	/* smooth normals: get average per triangle */
	Vector3 averageNormal;
	for(unsigned i = 0; i < index_buffer_data.size(); i += 3)
	{
		//get all 3 vertex normals
...
/*!
\brief
Generate quad with reference point at bottom left of quad
Then generate the VBO/IBO and store them in Mesh object
\param meshName - name of mesh
\param pos_X - width of quad
\param pos_Y - height of quad
\param width
\param height
\return Pointer to mesh storing VBO/IBO of quad
*/
...
Mesh* MeshBuilder::GenerateTileMap(const std::string &meshName, Color color, int width, int height, unsigned numRow, unsigned numCol)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
