...
float Vector3::getAngleFromDir(float dirX, float dirY) throw( DivideByZero )
{
	...
	/* 1st and 2nd Quater (Y >= 0) */
	if(dirY >= 0)
	{
		if(dirX > 0)
		{
			angle = Math::RadianToDegree(atan(xy));
			//std::cout << angle << std::endl;
		}
		else if(dirX < 0)
		{
			angle = 180 + Math::RadianToDegree(atan(xy));
		}
		else
		{
			angle = 90.f;
	...
	else if(dirY < 0)
	{
		...
		{
			angle = 360 + Math::RadianToDegree(atan(xy));
		}
		else
		{
			angle = 270.f;
